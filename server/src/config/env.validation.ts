...
export const envValidationSchema = Joi.object({
  ...
  DATABASE_URL: Joi.string()
    ...
    .when('NODE_ENV', {
      is: 'production',
      then: Joi.string()
        .uri({ scheme: ['postgresql', 'postgres', 'mysql', 'mongodb', 'mongodb+srv'] })
        .required(),
    })
    .messages({
      'string.uriCustomScheme':
        '"DATABASE_URL" in production must be a real database URI (postgresql://, mysql://, mongodb://, etc.), not a local file path',
    }),
