...
| `FarmerProfileUpdated` | `("FarmerProfileUpdated", farmer)` | `(farmer, name, timestamp, ledger_sequence)` | A farmer profile was updated directly via `update_farmer_profile`. |
...
| `CampaignMetadataUpdated` | `("CampaignMetadataUpdated", campaign_id)` | `(farmer, title, timestamp, ledger_sequence)` | Campaign metadata (title/description) was updated via `update_campaign_metadata`. |
