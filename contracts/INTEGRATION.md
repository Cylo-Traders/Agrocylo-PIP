...
| `FarmerUpdated` | `("FarmerUpdated", farmer)` | `(farmer, name, location, timestamp, ledger_sequence)` | An existing farmer profile was updated via `update_farmer_profile`. Distinct from `FarmerRegistered` so indexers can separate create vs update. |
...
| `CampaignMetadataUpdated` | `("CampaignMetadataUpdated", campaign_id)` | `(farmer, title, timestamp, ledger_sequence)` | Campaign title/description updated via `update_campaign_metadata`. Distinct from `CampaignRegistered`. |
